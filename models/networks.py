...
class ResnetGenerator_layer(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, 
        use_dropout=False, n_blocks=6, padding_type='reflect', n_downsampling=2):
        assert(n_blocks >= 0)
        super(ResnetGenerator_layer, self).__init__()
        ...
        for i in range(n_downsampling):
            ...
            model += [convlayer(ngf * mult, ngf * mult * 2, kernel_size=3,
        ...
        for i in range(n_downsampling):
            ...
            model += [deconv_layer(ngf * mult, int(ngf * mult / 2),
                                         kernel_size=3, stride=2,
                                         padding=1, output_padding=1,
                                         # bias=use_bias, fix=fix),
                                         bias=use_bias),
...
ResnetGenerator = ResnetGenerator_net
# ResnetGenerator = ResnetGenerator_layer
...
class ResnetBlockRe(nn.Module):
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        super(ResnetBlockRe, self).__init__()
        self.conv_block = self.build_conv_block(dim, padding_type, norm_layer, use_dropout, use_bias)
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [Conv_DCFre(dim, dim, kernel_size=3, padding=p, bias=use_bias),
        # conv_block += [Conv_DCFrw(dim, dim, kernel_size=3, padding=p, bias=use_bias),
                       norm_layer(dim),
                       nn.ReLU(True)]
        if use_dropout:
